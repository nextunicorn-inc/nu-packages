 ...
 */
export function cleanUpText(input?: string, substitutions?: MapOfStrings): string {
  if (!input) {
    return '';
  }
...
export function simpleFilterOptions(options: Option[], filter?: string): Option[] {
  // If the filter is blank, return the full list of Options.
  if (!filter) {
    return options;
  }
  ...
  return options.filter(
    ({ label, value }) =>
      label != null && value != null && label.toLowerCase().includes(filter.toLowerCase()),
  );
}
