...
## Notice
- 기존 아이콘 사용시, size props에 아무 값도 넣지 않고 쓰더라도 오류를 반환하비 않았지만, 0.1.xx버전 이후부터는 아이콘 size가 20이 없을 경우 에러를 반환합니다.
...
// 3.대소문자 변경된 사항
const onlyLetterChangedFileNamesTemplate = ({ overlappedOldFilenames, overlappedNewFilenames }) => {
  return `
## 3.대소문자 변경된 사항
<div style="display: flex;  justify-content: space-around">
  <div style="display: flex; flex-direction: column; justify-content: center; align-items: center">
...
const onlyOldFileNames = oldFileNames.filter(
  (fileName) => !intersectionByToLower.map((a) => a.toLowerCase()).includes(fileName.toLowerCase()),
);
